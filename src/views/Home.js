...
export const Home = () => {
    ...
    const onSubmit = async () => {
        ...
        console.log("start--------->", account, library)
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < collections.length; i++) {
            if (collections[i].name === collectionName) {
                ...
                if (chainId !== 10 || chainId !== 42161) setErc721(collections[i].platform)
                if (chainId !== 10 || chainId !== 42161) checkAllowd(collections[i].platform)
            }
        }
        if (exist) {
                ...
                <div className="relative w-full h-auto max-w-4xl p-4 mx-2 sm:mt-8">
                    <div className="relative my-auto bg-[#DCDAE9] rounded-3xl shadow-lg dark:bg-gray-700">
                        ...
                        <div className="flex flex-col py-2 mt-2 border border-transparent border-t-slate-400 border-t-1">
                            ...
                            {expand && <div className="flex flex-col justify-between w-full gap-6 px-6 py-2 sm:flex-row">
                                ...
                                <div className="flex flex-col items-center w-full gap-3 sm:w-auto">
                                    <p className="text-[#e48b24] h-[28px]">Join our email list for future updates</p>
                                    <MailchimpSubscribe
                                        url={postUrl}
                            ...
                            <ul className="py-1" aria-labelledby="dropdownButton">
                                {supportedChainIds.map((networkId) => {
                                    ...
                                    return (<li key={network.label}>
                                        <span onClick={() => handleSwitchNetwork(networkId)} className="flex items-center gap-2 px-4 py-2 text-lg text-gray-700 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white"><img className="max-h-[30px] rounded-full" src={network.image} alt={network.label} />{network.label}</span>
                                    </li>)
                                })}
                            </ul>
                        </div>}
                ...
                <div className="flex flex-col-reverse items-center max-w-5xl gap-4 px-6 mx-auto mb-8 md:flex-row">
                    <div className="flex flex-col items-center w-full">
                        ...
                        <div className="max-w-sm mt-8 bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <div className="flex flex-col gap-4 p-5">
                                ...
                                {description ? <pre className="font-normal text-gray-700 break-words whitespace-pre-wrap dark:text-gray-400">{description}</pre> :
                    ...
                    <div className="flex flex-col w-full">
                        <div className="flex items-center justify-center w-full mt-8">
                            ...
                            <span className="ml-3 mr-3 text-lg font-medium text-gray-900 dark:text-gray-300">Standard</span>
                            <label htmlFor="yellow-toggle" className="relative inline-flex items-center cursor-pointer">
                        ...
                        {/* File Upload */}
                        <label className="block mt-4 mb-2 text-sm font-medium text-gray-900 dark:text-gray-300" htmlFor="user_avatar">Upload file</label>
                        <input className="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none focus:border-transparent dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" aria-describedby="user_avatar_help" id="user_avatar" type="file" accept="image/png, image/gif, image/jpeg" onChange={readImage} />
                        {/* Title */}
                        <label htmlFor="title" className="flex items-center gap-1 mt-4 mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Title<img data-tooltip-target="tooltip-title" className="w-[15px] h-[15px]" src={info} alt="info" /></label>
                        ...
                        {advanced && <div className="flex gap-2 mt-4">
                            <div className="flex-col flex-grow">
                                <label htmlFor="collection" className="flex items-center gap-1 mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Collection<img data-tooltip-target="tooltip-collection" className="w-[15px] h-[15px]" src={info} alt="info" /></label>
                                ...
                                <div className="relative flex" ref={collection} id="collection">
                                    ...
                                    {openCollection && <div className="z-10 mr-2 text-base list-none bg-white w-full absolute top-[45px] rounded divide-y divide-gray-100 shadow dark:bg-gray-700">
                                        <ul className="w-full py-0" aria-labelledby="collectionField">
                                            {collections.filter(item => item.name.toLowerCase().includes(collectionName.toLowerCase())).map((item) => {
                                                // const network = NETWORKS[networkId]
                                                return (<li key={item.platform}>
                                                    <span onClick={() => handleSelectCollection(item)} className="flex items-center w-full gap-2 px-4 py-1 text-gray-700 cursor-pointer text-md hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">{item.name}</span>
                                                </li>)
                                            })}
                                        </ul>
                                    </div>}
                            ...
                            <div className="flex-col">
                                <label htmlFor="symbol" className="flex items-center gap-1 mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Symbol<img data-tooltip-target="tooltip-symbol" className="w-[15px] h-[15px]" src={info} alt="info" /></label>
                                ...
                                <input type="text" id="symbol" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#e48b24] focus:border-[#e48b24] block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-[#e48b24] dark:focus:border-[#e48b24]" onChange={(e) => setSymbol(e.target.value)} maxLength={4} value={symbol} disabled={!symbolEditable} />
                            </div>
                        </div>}
                        {/* Description     */}
                        <label htmlFor="message" className="flex items-center gap-1 mt-4 mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Description<img data-tooltip-target="tooltip-description" className="w-[15px] h-[15px]" src={info} alt="info" /></label>
                        ...
                        {/* Amount */}
                        <label htmlFor="amount" className="flex items-center gap-1 mt-4 mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Quantity <img data-tooltip-target="tooltip-amount" className="w-[15px] h-[15px]" src={info} alt="info" /></label>
                        ...
                        {/* Is for Sale     */}
                        {erc721 && advanced && <div className="flex items-center mt-4">
                        ...
                        {forSale && !approved &&
                            <button type="submit" className="flex items-center justify-center mt-8 text-white bg-[#e48b24] hover:bg-[#f7a531] focus:ring-4 focus:ring-[#f7b541] font-medium rounded-lg text-sm px-12 py-2.5 text-center dark:bg-[#f7a531] dark:hover:bg-[#f7b541] dark:focus:ring-[#f7b541]" onClick={signList} disabled={approving}>
                                {approving && <svg role="status" className="inline w-4 h-4 mr-3 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="#E5E7EB" />
                                    <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentColor" />
                                </svg>}
                                Approve
                        ...
                        {forSale && approved && <div className="flex">
                            ...
                            <input type="number" id="website-admin" className="mt-4 rounded-none rounded-r-lg bg-gray-50 border border-gray-300 text-gray-900 focus:ring-[#e48b24] focus:border-[#e48b24] block flex-1 min-w-0 w-full text-sm p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-[#e48b24] dark:focus:border-[#e48b24]" onChange={(e) => setBid(e.target.value)} />
                        </div>}
                        {/* Buy it now price */}
                        {forSale && approved && <div className="flex">
                            <span className="min-w-[128px] text-center mt-4 inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 rounded-l-md border border-r-0 border-gray-300 dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">
                            ...
                            <input type="number" id="website-admin" className="mt-4 rounded-none rounded-r-lg bg-gray-50 border border-gray-300 text-gray-900 focus:ring-[#e48b24] focus:border-[#e48b24] block flex-1 min-w-0 w-full text-sm p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-[#e48b24] dark:focus:border-[#e48b24]" onChange={(e) => setEndingPrice(e.target.value)} />
                        </div>}
                        {/* Auction Length     */}
                        {forSale && approved && <div className="flex">
                            <span className="min-w-[128px] text-center mt-4 inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 rounded-l-md border border-r-0 border-gray-300 dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">
                            ...
                            <input type="number" id="website-admin" className="mt-4 rounded-none bg-gray-50 border border-gray-300 text-gray-900 focus:ring-[#e48b24] focus:border-[#e48b24] block flex-1 min-w-0 w-full text-sm p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-[#e48b24] dark:focus:border-[#e48b24]" onChange={(e) => setDuration(e.target.value)} />
                        ...
                        <div className="flex items-center mt-6">
                            <input id="checkbox-3" aria-describedby="checkbox-3" type="checkbox" className="w-4 h-4 text-[#e48b24] bg-gray-100 rounded border-gray-300 focus:ring-[#e48b24] dark:focus:ring-[#e48b24] dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" onChange={(e) => setAgree(e.target.checked)} />
                            <label htmlFor="checkbox-3" className="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">
                                I agree to Bidify Mint's <a className="text-[#e48b24]" href={Terms} rel="noopener noreferrer" target="_blank" >Terms &amp; Conditions</a> and <a className="text-[#e48b24]" href={Policy} rel="noopener noreferrer" target="_blank" >Privacy Policy</a>.
                            </label>
                        </div>
                        <div className="flex items-center justify-center gap-1 mt-4"><label className="self-center hidden sm:block text-[#e48b24]">fee = {Number(cost) > 0 ? `${ethers.utils.formatEther(cost)} ${getSymbol()}` : 'N/A'}</label>
                            <img data-tooltip-target="tooltip-fee" className="w-[15px] h-[15px] hidden sm:block" src={info} alt="info" /></div>
                            ...
                            <div className="relative w-full h-auto max-w-md px-4">
                                <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    <div className="p-6 pt-16 pb-8 text-center">
                                        ...
                                        <h3 className="mt-3 mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">{modalContents[modalContent]}</h3>
                                    </div>
                                </div>
                            </div>
                        </div>}
